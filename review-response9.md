...
Verified against `taskbar-recent-focus-highlight.wh.cpp` v0.9.11.

---

## Copied review (required + optionals)

**1.** Unique-title fallback (`MatchTitleToUnusedRecent`) is handed every confirmed window on the desktop. Score 91/93 substring can bind a Chrome `GitHub` card to an Edge `GitHub - Profile` window. Pass 4 already filters by PID; pass 3 should too. Skip pass 3 when nothing resolved exactly.

**2.** `indexBindTrusted` starts as `sizesAgree` and only flips false on a DataContext disagreement. If no card resolves by DataContext, GetAt runs against a possibly stale `g_TaskGroup_Thumbnails`. Clear that global on the way *in* to `TargetItemKey`.

**3.** Min-focus / preview min-focus re-arm 200 ms forever while a transient window holds FG (`IsTransientForeground` includes this explorer.exe, so File Explorer). Bound the retries.

Optionals: snap-group vector once; keep overlay host across paints; deadline-style full-rebind debounce; `WH_MOD_VERSION` init log; `weak.get()` vs `HasThreadAccess` comment mismatch.

Functionality: File Explorer never highlighted; unique-title keep/drop left to human; `StopDecayTimerIfIdle` extra cycle.
...
- Anchors on any sibling that resolved by TaskItem or repeater.
- Filters `recent` to that `processKey`. Hosted UWP also requires the same AppUserModelID (AFH is shared, same as icons).
- Skips entirely when nothing resolved exactly (`Preview resolve: skip unique-title`).

Wrong glow on a card is worse than a blank flyout when the exact paths missed. Unique-title stays as the last hole-filler for the *same* app, with the note you asked for last round.
...
- **Keep overlay host.** Agreed — ranked repaint restores the plate brush and hides children; `XamlReader::Load` only when the host is missing. Unload / unranked still full-clear.
...
| # | Verdict | Work |
|---|---------|------|
| 1 unique-title process | Take | `processKey` + AUMID; skip if no exact sibling |
| 2 stale Thumbnails | Take | Clear on TargetItemKey entry |
| 3 200 ms forever | Take | Grace = min-focus (min 2s), then drop |
| Opt snap vector | Take | `vector<char> isSnap` |
| Opt overlay host | Take | `ResetThumbnailOverlayForRepaint` |
| Opt debounce | Take | skip SetTimer if armed |
| Opt WH_MOD_VERSION | Take | init log |
| Opt weak.get thread | Skip | one Explorer UI thread |
| Fn File Explorer | Take | CabinetWClass ranked |
| Fn unique-title drop | Keep + scoped | human reviewer |
| Fn extra decay cycle | Partial | StopDecayTimerIfIdle on give-up |
